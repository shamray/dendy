...
template <bus bus_t>
auto cpu<bus_t>::read_word(std::uint16_t addr) const -> std::uint16_t {
    auto lo = read(addr);
    auto hi = read(addr + 1);
...
template <bus bus_t>
auto cpu<bus_t>::read_word_wrapped(std::uint16_t addr) const -> std::uint16_t {
    auto lo = read(addr);
...
template <bus bus_t>
auto cpu<bus_t>::decode(std::uint8_t opcode) -> instruction {
